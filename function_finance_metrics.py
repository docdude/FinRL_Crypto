...
def write_metrics_to_results(name, file_path, drl_cumrets, drl_annual_ret, drl_annual_vol, drl_sharpe_rat, vol,
                             append_write):
    # Use .iloc[-1] for pandas Series, [-1] for lists/arrays
    last_cumret = drl_cumrets.iloc[-1] if hasattr(drl_cumrets, 'iloc') else drl_cumrets[-1]
    with open(file_path, append_write) as f:
        f.write('\n################################## ' + name + ' ####################################\n')
        f.write('Cumulative return:           ' + str(last_cumret * 100) + '\n')
